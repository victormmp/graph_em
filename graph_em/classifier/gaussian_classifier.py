...
from scipy.stats import beta, binom, multivariate_normal
...
def pseudo_det(m, threshold: float = 0):
    """
    Calculate the pseudo determinant of a matrix. The pseudo determinant is the product of all non-zero eigenvalues of
    a square matrix. If the matrix is non-singular, then the pseudo determinant coincides with the regular determinant.

    https://en.wikipedia.org/wiki/Pseudo-determinant

    Args:
        m: the square matrix.
        threshold: For computational equivalence, a threshold can be given which values bellow it are considered zero.
    Returns:
        The pseudo determinant of the square matrix.
    """
    eigenvalues = np.linalg.eig(m)[0]

    return np.product(eigenvalues[eigenvalues > threshold])


def likelihood(x: np.ndarray, mean: np.ndarray, sigma: np.ndarray, numpy_method: bool = False):
    """
    ...
    Args:
        ...
        numpy_method: Boolean - if true, use numpy implementation of multivariate gaussian pdf.
    Returns:
    ...
    if numpy_method:
        ...
        try:
            ...
            raise e
        return 1 / ((((2 * np.pi) ** n) * np.linalg.det(sigma))**0.5) * np.exp(-0.5 * term)
...
def calc_statistics(x):
    ...
    sigma_1 = np.cov(x.T)
    # sigma_1 = cov_matrix(x)

    return mean_1, sigma_1


def cov_matrix(x: np.ndarray) -> np.ndarray:
    std = np.std(x, axis=0).reshape([x.shape[1], 1])
    cov = np.corrcoef(x.T) * np.matmul(std, std.T)
    return cov
...
class ExpectationMaximization:
    ...
    def __init__(
            ...
            num_neighbors_start: int = 100,
            start_method: str = 'single'
    ):
        """
        ...
        Args:
            ...
            num_neighbors_start: Number of neighbors to start algorithm clusters in cold start.
        """
        self.n_clusters = n_clusters
        self.max_iter = max_iter
        self.threshold = threshold
        self.num_neighbors_start = num_neighbors_start
    ...
    def start_clusters(self, x: np.ndarray, statistics: List = None, method: str = 'single'):
        """
        ...
        Args:
            ...
            method: Method to obtain the cluster statistics:
    ...
    def fit(self, x: np.ndarray, statistics: List = None):
        ...
        self.start_clusters(x, statistics, self.start_method)
        tau = []
        error = np.inf
...
class GaussianMixture:
    ...
    def __init__(self, clusters: List = (3, 3), threshold=0.01, em_method: str = 'single'):
        """
        ...
        Args:
            ...
            threshold: Error threshold for EM algorithm.
        """
    ...
    @staticmethod
    def get_statistics(x: np.ndarray, k: int, threshold: float = 0.01, em_method: str = 'single') -> List:
        """
        ...
        Returns:
            ...
            cluster probability.
        """
        em = ExpectationMaximization(k, threshold=threshold, start_method=em_method)
        em.fit(x)
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray):
        ...
        logging.info(f'Getting statistics for class {labels[0]}')
        self.statistics_c1 = self.get_statistics(x_train[y_train == labels[0], :], self.clusters[0], self.threshold, self.em_method)
        logging.info(f'Getting statistics for class {labels[1]}')
        self.statistics_c2 = self.get_statistics(x_train[y_train == labels[1], :], self.clusters[1], self.threshold, self.em_method)
